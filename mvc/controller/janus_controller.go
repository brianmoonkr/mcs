...
func (c *JanusController) GetMessage() {
	...
	hostPort, err := c.getHostPort(param.EndPoint)
	if err != nil {
	...
	//targetUrl = fmt.Sprintf("%s/%s/_search?sort=date:desc&_source_includes=room,text,from,date&q=room:%s", targetUrl, hostPort, param.RoomId)

	resp, err := client.R().
		SetHeader("Content-Type", "application/json").
		Get(fmt.Sprintf("%s_%s/_search?sort=indexed_at:asc&_source_includes=room,text,from,date,indexed_at&q=room:%s", targetUrl, hostPort, param.RoomId))

	if err != nil {
		res.Error400(c.CTX, err, "C : JanusController::GetMessage() - destroy video room")
		return
	}

	log.Println("=== C : JanusController::GetMessage() ===")
	log.Println(resp)
...
func (c *JanusController) RemoveRoom() {
	...
	*/
	...
	if live.Status == "9000" {
		...
		res.ErrorMessage(c.CTX, 901, "Already terminated")
		return
	}
...
*/
	...
	log.Println("=== destroy videoroom room ===")
	log.Println(resp)
	...
	go TranscodeBySession(liveSeq, live.Videoroom, hostPort, live.Callback)
...
func (c *JanusController) GetParticipantList() {
	...
	*/
	...
	log.Println("=== room participant list ===")
	log.Println(resp)
...
func (c *JanusController) KickParticipant() {
	...
	log.Println("=== get result for kicking participant ===")
	log.Println(resp)
...
func (c *JanusController) RecreateRoom() {
	...
	*/
	...
	if exists {
		log.Println("################################# Videoroom id Already exists ###################################")
		res.Send(c.CTX, "Videoroom id Already exists")
		return
	}
	// mariadb update
	janusVO := &vo.JanusVO{
	...
	reqBody.Secret = "teamgrit"
	reqBody.Post = config.CF.Prop.TextroomPost + "_" + hostPort + "/" + hostPort
	...
	log.Println("=== recreate video room ===")
	log.Println(resp)
	...
	log.Println("=== recreate text room ===")
	log.Println(resp)
...
func (c *JanusController) CreateRoom() {
	...
	*/
	...
	hostPort, err := c.getHostPort(c.WebsocketConn)
	if err != nil {
...
// function :: destroy janus session
func (c *JanusController) DestroyJanusSession(sessionId uint64) (JanusCreate, error) {
	client := resty.New()
	tr := strconv.FormatInt(time.Now().Unix(), 10)
	...
	if c.CurrentSessionId == sessionResp.SessionId {
...
func (c *JanusController) AttachJanusPlugin() (JanusCreate, error) {
	...
	log.Println("=== create videoroom plugin ===")
	log.Println(resp)
	...
	log.Println("=== create textroom plugin ===")
	log.Println(resp)
...
/*
	...
	if !isValid {
		...
		remain := ts % int64( len(mediaServers) )
		server := mediaServers[remain]
		mediaConns := strings.Split(server, "$$")
	...
	*/
	...
	for _, mediaServer := range mediaServers {
		if strings.Contains(mediaServer, wss) {
			isValid = true
			mediaConns := strings.Split(mediaServer, "$$")
...
func (c *JanusController) getHostPort(strUrl string) (string, error) {
	u, err := url.Parse(strUrl)
	if err != nil {
		log.Fatal(err)
		return "", err
...
func (c *JanusController) doesRoomExist(paramEndPoint string, paramRoomId string) (bool, error) {
	...
	log.Println("=== check if video room exists ===")
	log.Println(resp)
